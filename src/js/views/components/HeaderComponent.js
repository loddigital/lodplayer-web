export const getHeaderHTML = (isHome = true) => {
    const navLinks = isHome 
        ...
        : `<a href="javascript:void(0)" onclick="if(window.history.length > 1 && document.referrer.includes(window.location.host)) { window.history.back(); } else { window.location.href='{{seo.base_url}}'; }" class="text-[0.95rem] font-medium text-textMuted hover:text-white transition-all flex items-center gap-2">
    ...
    <header id="header" class="fixed top-0 left-0 w-full z-100 backdrop-blur-lg bg-bgDark/70 border-b border-white/12 transition-all duration-300">
        <div id="nav-container" class="max-w-300 mx-auto px-4 md:px-8 relative flex justify-between items-center h-[70px] md:h-[80px] transition-all duration-300">
            <a href="{{seo.base_url}}" class="flex items-center gap-2 md:gap-3 text-xl md:text-2xl font-extrabold font-outfit bg-clip-text text-transparent bg-[linear-gradient(135deg,#60a5fa_0%,#3b82f6_100%)]">
